...
class DummyTrade:
    ...
    def __init__(self, strike: int, opt_type: str, direction: str, entry_price: float, entry_time: str):
        self.strike = strike
...
class IntradayDummyEngine:
    ...
    def run_day(self, date_str: str):
        ...
        print(f"Loaded Master Clock: {len(clock)} ticks in {(time.time() - t0)*1000:.2f}ms")
        ...
        for loop_time, spot_price in clock.items():
            ...
            for trade in self.open_positions[:]:
                ...
                if pnl <= -20: # Stop Loss Trigger
                    self.log(loop_time, f"EXIT {trade} at {current_leg_price}. PNL: {pnl:.2f}")
                    self.open_positions.remove(trade)
                    # For dummy sake, let's reverse direction immediately on stop out
                    self.enter_trade(date_str, loop_time, spot_price, trade.opt_type, 'BUY')
    ...
    def enter_trade(self, date_str: str, time_str: str, spot_price: float, opt_type: str, direction: str):
        ...
        if cache_key not in self.cache:
            t1 = time.time()
            data = self.loader.get_instrument_data(date_str, atm, opt_type, time_str)
            self.cache[cache_key] = data
            self.log(time_str, f"LAZY FETCH: {cache_key} fetched {len(data)} bars to cache in {(time.time()-t1)*1000:.2f}ms")
        ...
        try:
            ...
            self.log(time_str, f"FAILED TO ENTER: No price data for {cache_key} at {time_str}")

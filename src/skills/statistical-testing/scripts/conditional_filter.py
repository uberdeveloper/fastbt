...
class ConditionParser:
    ...
    def parse_multiple_conditions(
        ...
        if len(set(logic_operators)) > 1:
            raise ValueError(
                "Mixed AND/OR logic not supported. "
                "Use either all AND or all OR."
            )
...
class DataSegmenter:
    ...
    def validate_conditions(
        ...
        for cond in conditional_filter.conditions:
            ...
            if cond.column_type == 'numeric':
                ...
                if not pd.api.types.is_string_dtype(col_dtype) and col_dtype != 'object':
                    warnings.append(
    ...
    def segment_data(
        ...
        if len(group_true) == 0:
            raise ValueError(
                f"Condition matches 0 observations. "
                f"Please adjust condition thresholds."
            )
        
        if len(group_false) == 0:
            raise ValueError(
                f"Condition matches ALL observations. "
                f"No comparison group available."
            )
...
if __name__ == "__main__":
    ...
    cond_filter = parser.parse_multiple_conditions(
        "VIX > 20 AND market_cap > 1000"
    )
    print(f"Parsed {len(cond_filter.conditions)} conditions with {cond_filter.logic} logic")
    print()

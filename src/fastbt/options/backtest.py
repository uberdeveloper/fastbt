...
class OptionsBacktest:
    ...
    def generate_options_table(self, contracts:List[Tuple[str,str,int]])->pd.DataFrame:
        """
        Given a list of contracts, generate the options table
        contracts
            list of 3-tuples in the form ('BUY', 'CALL', 0)
        """
        prices = self.data.set_index('timestamp').between_time(self.start, self.start).spot
        option_strikes = []
    ...
    def get_result(self, data):
        res = pd.DataFrame.from_records(data, columns=['side', 'entry_time', 'entry_price', 'exit_time', 'exit_price'],
                index=data.index)
    ...
    def run(self, contracts):
        ...
        """
        def f(x,stop=100):
            tb = self.tradebook(x.open.values, high=x.high.values, low=x.low.values, close=x.close.values,
                    timestamp=x.timestamp.values, order=x.side.values[0], stop=stop)
            return tb
        option_strikes = self.generate_options_table(contracts)
        run_data = self.data.set_index('timestamp').between_time(self.start, self.end).reset_index()
